...
class CommandeClientControllerTest {
    ...
    void getAll_shouldReturn200WithList() throws Exception {
        ...
        Page<CommandeClientResponse> page = new PageImpl<>(responses);
        when(commandeClientService.getAll(any(Pageable.class))).thenReturn(page);

        mockMvc.perform(get("/commandes-client"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(1))
                .andExpect(jsonPath("$.content[0].codeCommande").value("CC-2026-0001"));
    }
